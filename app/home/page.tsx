...
const { t, lang } = useLang();
  ...
  return (
    <div className="shell home-page">
      {toast && (
        <div style={{ position: 'fixed', bottom: 'calc(90px + var(--sab))', left: '50%', transform: 'translateX(-50%)', background: '#111', color: '#fff', padding: '10px 20px', borderRadius: 20, fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 600, zIndex: 9998, whiteSpace: 'nowrap', pointerEvents: 'none' }}>
          {toast}
        </div>
      )}
      {showSplash && (
      ...
      <header className="home-header">
        ...
        <div className="header-side">
          ...
          <button className="icon-btn" style={{ position: 'relative' }} onClick={() => { setHasUnread(false); if (myDbId) supabase.from('notifications').update({ read: true }).eq('user_id', myDbId).eq('read', false).then(() => {}); router.push('/notifications'); }}>
      ...
      {showScrollTop && (
        <button onClick={() => { feedScrollRef.current?.scrollTo({ top: 0, behavior: 'smooth' }); setShowScrollTop(false); }}
          style={{ position: 'fixed', bottom: 'calc(88px + var(--sab))', left: '50%', transform: 'translateX(-50%)', zIndex: 20, width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.92)', border: '1px solid #E0E0E0', boxShadow: '0 2px 8px rgba(0,0,0,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
