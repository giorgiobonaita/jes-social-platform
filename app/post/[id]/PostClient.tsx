...
export default function PostClient({ id }: { id: string }) {
  ...
  const handleShare = async () => {
    const url = `https://jessocial.com/post?id=${id}`;
    try {
      if (navigator.share) {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#F5F5F5', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      {/* Header */}
      <header style={{ width: '100%', maxWidth: 430, background: '#fff', borderBottom: '1px solid #EEEEEE', padding: '16px 20px', paddingTop: 'calc(16px + var(--sat))', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
